...
library(gridExtra)
#load data
data<-read.csv("/Users/christinasun/Desktop/google/googleplaystore.csv")
```
...
highchart() %>%
  hc_chart(type = "pyramid",labels = content_rating$Content.Rating, values = content_rating$Total.Reviews) %>%
  hc_title(
    text = "Number of reviews by Content Rating"
  ) %>%
  hc_add_theme(hc_theme_flat())
```
...
model.1<-lmer(log_reviews ~ Rating + log_installs + Size + (1|Category), data = newdata)
summary(model.1)

#run random intercept and slope model
model.2<-lmer(log_reviews ~ Rating + log_installs + Size + (1+Rating|Category) + (1+log_installs|Category) + (1+Size|Category), data = newdata)
summary(model.2)

#run random slope model
model.3<-lmer(log_installs ~ Rating + log_reviews + Size + (Rating|Category) + (log_reviews|Category) + (Size|Category), data = newdata)
summary(model.3) 

anova(model.1,model.2,model.3) # choose 'model.2'
